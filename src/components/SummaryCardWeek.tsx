...
interface SummaryCardWeekProps {
  totalAmount: string
  dailyGoal: string
}

export function SummaryCardWeek({
  totalAmount,
  dailyGoal,
}: SummaryCardWeekProps) {
  const { monthSelected } = useSelectedDateStore()
  ...
  return (
    <div className="bg-zinc-50 w-[100px] h-24 rounded-lg shadow-sm p-2 flex flex-col justify-between">
      <div className="flex items-center justify-between w-full">
        <span className="text-xs font-light">Week</span>
